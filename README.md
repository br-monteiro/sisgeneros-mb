...
$ cd ~/sisgeneros-mb
```
2. Após entrar no diretório do projeto, é necessário fornecer permissão de escrita no arquivo `~/sisgeneros-mb-master/App/Database/DbRepository/sisgeneros.db`. Este processo pode ser feito de várias formas, porém, apenas alterar o **owner group** do arquivo já deve bastar.
```bash
...
customer care
 ...
 +55 48 3232.3200  +55 91 9 8509.2695 
 Av. Afonso Delambert Neto, 26 - Lagoa da Conceição
Florianópolis, SC - Brasil - 88062-000  
- Apache Server 2.4+
  - mod_rewrite
  - libapache2-mod-php
- PHP 7.0+
...
$ cd ~/
```
Caso você tenha optado por baixar o arquivo zipado da ultima versão, descompact o mesmo e entre no diretório criado por este processo.
```bash
$ cd ~/sisgeneros-mb
```
...
$ php setup.php
```
Se tudo ocorrer com sucesso, a seguinte saida deve ser observada no terminal:
```bash
$ php setup.php
> Chave SALT alterada com sucesso
> Path do Core alterado com sucesso
...
#### Geração de PDF
A aplicação faz uso de um binário que auxilía na criação de arquivos PDF pela biblioteca `knp-snappy` (já presente no sistema). Este binário é o [wkhtmltopdf](https://wkhtmltopdf.org/) e encontra-se no path `~/sisgeneros-mb-master/vendor/h4cc/wkhtmltopdf-i386/bin/wkhtmltopdf-i386`. Será necessário criar um link simbólico dentro de `/usr/bin/`.
```bash
